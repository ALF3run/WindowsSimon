window.onload = function() {
  ...
  document.getElementById('strict').addEventListener('click', function() {
    ...
    if (strict === false) {
      ...
      document.body.style.background = 'linear-gradient(#F9A825 10%, #FFEE58 80%, #FFF59D)';
    } else {
      strict = false;
  ...
  function chkSequence(sq) {
    ...
    if (playerMoves === moves[j]) j++;
      ...
      setTimeout(function() {    
        if (strict) document.body.style.background = 'linear-gradient(#F9A825 10%, #FFEE58 80%, #FFF59D)';    
        else document.body.style.background = 'linear-gradient(#1565C0 10%, #42A5F5 80%, #90CAF9)';
      }, 1000);
      if (strict === true) {
